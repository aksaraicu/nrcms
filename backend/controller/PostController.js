...
export const createPost = async (req, res) => {
  ...
  let format;
  await sharp(data).resize(400).toFile(`./public/gambar/${fileName}.webp`).then(data => {
    format = data.format;
  })
  const url = `${req.protocol}://${req.get("host")}/gambar/${fileName}.${format}`;
  try {
    await Post.create({
      judul: judul,
      isi: isi,
...
export const updatePost = async (req, res) => {
  ...
  if (req.files === null) {
    ...
    const ext = path.extname(gambar.name);
    const allowwedType = [".png", ".jpg", ".jpeg", ".webp"];
    if (!allowwedType.includes(ext.toLowerCase())) return res.status(422).json({ msg: "Ekstensi gambar salah" });
    fileName = gambar.md5;
  ...
  const url = `${req.protocol}://${req.get("host")}/gambar/${fileName}${format ? '.' + format : ''}`;
  try {
    await Post.update({
      judul: judul,
      isi: isi,
